...
export const admissionScannerService = {
  // Debug function to help troubleshoot activity lookup issues
  async debugActivityLookup(activityId) {
    console.log(`=== DEBUG: Activity Lookup for ID: ${activityId} ===`);
    ...
    try {
      ...
      console.log(
        `1. Document ID lookup: ${
          activitySnap.exists() ? "FOUND" : "NOT FOUND"
        }`
      );
      ...
      console.log(
        `2. 'id' field lookup: ${!idSnapshot.empty ? "FOUND" : "NOT FOUND"} (${
          idSnapshot.size
        } results)`
      );

      idSnapshot.forEach((doc, index) => {
        const data = doc.data();
      ...
      // Check by 'activityId' field
      const qActivityId = query(
        collection(db, "activities"),
        where("activityId", "==", activityId),
        limit(5)
      );
      const activityIdSnapshot = await getDocs(qActivityId);
      console.log(
        `3. 'activityId' field lookup: ${
          !activityIdSnapshot.empty ? "FOUND" : "NOT FOUND"
        } (${activityIdSnapshot.size} results)`
      );

      activityIdSnapshot.forEach((doc, index) => {
        const data = doc.data();
        console.log(`   Result ${index + 1}:`, {
          docId: doc.id,
          activityId: data.activityId,
      ...
      // List recent activities for comparison
      ...
      console.log(`4. Recent activities for comparison:`);
