...
export const studentsService = {
  ...
  async registerDepartment(departmentName, collectionName, creatorInfo) {
    try {
      ...
      // Register new department with comprehensive data
      const departmentDoc = {
        name: departmentName,
      ...
      console.log(`Department ${departmentName} registered in stats with ${initialStats.totalStudents} students`);
    } catch (error) {
  ...
  async updateDepartmentStats(departmentName) {
    try {
      ...
      // Update the first matching document with comprehensive stats
      const deptDoc = querySnapshot.docs[0];
      await updateDoc(deptDoc.ref, {
      ...
      console.log(`Updated stats for ${departmentName}: ${stats.totalStudents} total, ${stats.activeStudents} active, ${stats.totalStudents - stats.activeStudents} inactive`);
      return stats;
    } catch (error) {
  ...
  async getDepartmentsOverview() {
    try {
      console.log('getDepartmentsOverview: Querying stats collection...');
      ...
      const querySnapshot = await getDocs(q);
      console.log('getDepartmentsOverview: Found', querySnapshot.size, 'departments in stats');
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data.totalStudents > 0) {
          console.log('Department from stats:', data.name, 'Students:', data.totalStudents);
          const dept = {
            id: doc.id,
            name: data.name,
            collectionName: data.collectionName,
      ...
      console.log('getDepartmentsOverview: Returning result:', result);
      return result;
    } catch (error) {
  ...
  getDepartmentCollectionName(department) {
    ...
    const mapping = {
      ...
      "Computer Science and Management": "students_csm",
